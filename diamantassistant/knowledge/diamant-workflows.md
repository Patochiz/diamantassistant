# Process internes DIAMANT INDUSTRIE

> Ce fichier sera injecté automatiquement dans le prompt système de l'assistant.
> Documentez ici vos workflows maison, vos conventions de nommage, vos règles métier.

## Création d'un nouveau produit

**Règles à rappeler systématiquement :**
- Toujours **vérifier d'abord** que le produit n'existe pas (recherche par référence OU libellé OU partie du libellé).
- Créer **d'abord le produit à la vente** (fiche produit complète avec description).
- Puis, dans l'onglet **Fournisseurs** de la fiche produit, ajouter le ou les produits fournisseurs associés.
- **Ne jamais** créer un produit fournisseur isolé.
- Renseigner systématiquement la **description** (pas seulement la référence), elle apparaîtra sur devis, commandes et factures.
- Associer les **catégories** appropriées pour les statistiques et le filtrage.

## Conventions de nommage

<!-- À remplir : références produits, codes clients, catégories... -->

## Workflow devis → commande → facture

<!-- À remplir -->

## Plafonds métalliques suspendus — spécificités

<!-- À remplir : particularités métier, configurations types... -->

## Sous-traitance métallique — spécificités

<!-- À remplir -->

## Qui contacter pour quoi

- **Patrice** : administration Dolibarr, bugs, demandes d'évolution, questions techniques.
- <!-- À compléter : autres référents DIAMANT -->
